...
class UserStates(StatesGroup):
    ...
    # Можно добавить другие состояния с привязкой к языку

# Храним язык пользователя (в реальном боте - в БД)
user_languages = {}

@router.message(Command("start"))
async def start_handler(message: Message, state: FSMContext):
    await message.answer(
        get_text("ua", "start_choose"),  # Стартовое сообщение всегда на украинском
        parse_mode="HTML",
...
async def process_agreement(callback: CallbackQuery, state: FSMContext):
    ...
    # Сохраняем язык пользователя в queue_manager
    from services.queue_manager import queue_manager
    queue_manager.user_languages[user_id] = lang
    
    # Также сохраняем в FSM state
...
async def callback_subscribed(callback: CallbackQuery):
    lang = user_languages.get(callback.from_user.id, "ua")
    alert_text = get_text(lang, "subscribed_alert")
    await callback.answer(alert_text, show_alert=True)

# Функция для получения текста дисклеймера (остается без изменений)
def get_disclaimer_text(language):
    # ... ваша существующая функция с текстами дисклеймера
    texts = {
